#!/usr/bin/env python3
"""
หา Setting ที่ให้ความแน่นอนสูงสุด v3.1

เน้น:
- Win Rate สูงสุดเท่าที่ทำได้ (>75%)
- เลือกแค่ TOP 1-2 ตัวต่อสัปดาห์
- Volume Confirmation (Trend + Accumulation)
- Batch Download (เร็วมาก)
...
def sim_trade(df, idx, stop_loss=5, target=10, maxhold=30):
    ...
    # Check gap down on day 1
    if idx + 1 < len(df):
        open_day1 = df.iloc[idx+1]['Open']
        gap_down = ((open_day1 - entry) / entry) * 100
...
def run():
    print("=" * 70)
    print("หา Setting ที่ให้ความแน่นอนสูงสุด v3.1 (Batch Download)")
