# resonance / evidence — the measured record behind every rule in decide.md

**This file is REFERENCE, not the morning instruction.** `decide.md` is what you follow; it states each
gate as a decidable test and stays short enough to be read in full. Everything here is the WHY: the
measurements, the worked examples, and the losses each rule was bought with.

**Read a section of this file when** a gate's answer is genuinely unclear, when you want to check
whether a claim still holds on today's tape (you have Bash — re-derive it), or when you are about to
override a rule and want to know what it cost to learn. **Do not read it end-to-end every morning.**
It was split out on 2026-09-04 because decide.md had reached 721 lines and memory.md 995: about 1,700
lines to read before any research could start, which is more than gets read — and a rule that is
...
2. **Direction-flow gate** ("who buys at my open, and why would they pay UP?"): the plan NAMES the
   ...
   **To clear this gate the plan must answer, in words: "after 09:30, WHO still has to buy — and why
   ...
   (c) **The CAUSE is not finished yet** — the thing driving the name still has un-run steps ahead of it.

   **⚠️ HOW TO DECIDE (c) — THIS IS THE MOST MISREAD LINE IN THIS DOCUMENT, SO IT IS SPELLED OUT.**
   "Consumed" is a property of the CAUSE, never of the PRICE. To judge it you must **name the event that
   would FINISH this cause, and say whether it has already happened.** That is the whole test:
     - You CAN name a completing event that has occurred → **CONSUMED**, even if the price has barely
       moved (the PT ladder has all printed; the number is out and modelled; the squeeze has covered;
       the deal closed).
     - You CANNOT → **NOT consumed**, even if the price has already run hard for several sessions (the
       legislation has not passed yet; the contract's revenue has not been modelled into estimates; the
       qualification has not converted to orders; the ladder is still climbing).
   **These are INADMISSIBLE as evidence that a cause is finished, on their own:** "it is up N% already",
   "it has run three sessions", "it gapped up", "the premarket already moved". Gap-% is measured NOT to
   separate these (a +14% gapper HELD; a +15% and a +5% both faded), and "extended" is a statement about
   price, not about the cause. If your only reason for declining is that the move is large or several days
   old, you have not applied this gate — you have substituted a price rule for it, which is the exact
   habit this system was rebuilt to remove.
   **Also ask the mirror question:** if the price has fallen back since the catalyst, WHY did it fall? A
   give-back caused by the MARKET (an index-wide down day, the sector, the underlying) leaves the cause
   fully intact and hands you a cheaper entry into the same unfinished thesis; a give-back caused by the
   NAME (the market read the news and sold it) is the market telling you the cause is weaker than the
   headline. Same price action, opposite meaning — and only the second is a reason to decline.
   ...
   **Do NOT collapse this into "company number = good, theme/attention = skip"** — that bucket is too
   ...
   symmetric across sources — but judge it by (c)'s test, the completing EVENT, never by how long or how
   far the move has run: a squeeze that has actually covered has no fuel; a theme whose driving event has
   already landed and been modelled is finished no matter how recently it started; and a theme that has run
   several sessions on a driver with steps STILL AHEAD of it is not finished merely because it has run. **Absence of sellers is not presence of buyers** — "nobody
...
3. **Gap-direction gate** (judge with the gap bullet below).
   ...
   **⚠️ AND RE-CHECK EDGAR AT ~09:20 ON ANY UP-GAPPER YOU CANNOT EXPLAIN — the 09:00-09:20 window is a
   dense disclosure band and a brain that freezes at 09:00 is systematically blind to it.** Measured on
   one replay session: THREE of 32 pooled names filed or released material news inside 09:00-09:20 — an
   antitrust settlement 8-K accepted at 09:04:20 (newswire 09:18) on a name whose unexplained +2.2%
   premarket gap had been dismissed as "a thin mark with nothing behind it", and a storage/software
   agreement at 09:05. The thin mark was anticipation of a disclosure that had not printed yet. So an
   unexplained up-gap is not automatically noise — it can be front-running a filing that lands minutes
   after your first pass. Before you commit, run `python -m tools.edgar.edgar <SYM>` once more on the
   up-gappers you could not name, and say in the plan what the re-check found (including "nothing new").
   It costs one call and it is the difference between "unreadable" and "readable ten minutes later".
