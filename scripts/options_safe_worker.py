"""Safe options worker — single key, 15s delay."""
...
def api(url):
    full = f"{url}&apiKey={API_KEY}" if '?' in url else f"{url}?apiKey={API_KEY}"
    for attempt in range(3):
        try:
            r = requests.get(full, timeout=30)
            if r.status_code == 429:
                print(f"  rate limit, wait 60s", flush=True)
                time.sleep(60)
                continue
            time.sleep(15)
            return r
...
for si, sym in enumerate(SYMBOLS):
    ...
    if existing > 10:
        print(f"[{si+1}/{len(SYMBOLS)}] {sym}: skip ({existing} exists)", flush=True)
        continue
    ...
    for ctype in ['call','put']:
        for exp in ['2025-01-01','2025-07-01','2026-01-01']:
            ...
            for c in r.json().get('results',[])[:7]:
                r2 = api(f"https://api.polygon.io/v2/aggs/ticker/{c['ticker']}/range/1/day/2024-04-01/2026-04-16?limit=50000")
                if not r2: continue
                n_calls += 1
    ...
    eta = elapsed/(si+1)*(len(SYMBOLS)-si-1)
    print(f"[{si+1}/{len(SYMBOLS)}] {sym}: {len(daily)} days, {n_calls} calls ({elapsed:.1f}m, ETA {eta:.0f}m)", flush=True)

total = conn.execute('SELECT COUNT(DISTINCT symbol), COUNT(*) FROM options_polygon_history').fetchone()
print(f"\nDone! {total[0]} symbols, {total[1]} rows ({(time.time()-t0)/60:.1f}m)")
conn.close()
