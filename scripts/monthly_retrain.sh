...
$PYTHON backtests/feature_builder.py --start $START --end $END --output /tmp/bt_features_v27.pkl --limit 500 >> $LOG 2>&1

# Symlink so train_v22 finds v22 pkl
ln -sf /tmp/bt_features_v27.pkl /tmp/bt_features_v22.pkl
...
echo "[$(date)] Train MFO-zone models (Z1/Z2/Z3/Z4)..." >> $LOG
$PYTHON scripts/train_zones.py --end-date $END --pkl /tmp/bt_features_v27.pkl >> $LOG 2>&1
...
echo "[$(date)] Restart engine to pick up new models..." >> $LOG
