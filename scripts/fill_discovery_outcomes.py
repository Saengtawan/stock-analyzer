    ...
    with get_session() as session:
        ...
        for row in rows:
            ...
            try:
                ...
                set_parts = []
                params = {}
                for i, (col, val) in enumerate(outcomes.items()):
                    set_parts.append(f'{col} = :v{i}')
                    params[f'v{i}'] = val
...
def fill_benchmarks():
    ...
    with get_session() as session:
        ...
        for col in ['benchmark_xlu_5d', 'benchmark_xle_5d', 'benchmark_spy_5d']:
            try:
                session.execute(text(f"ALTER TABLE discovery_picks ADD COLUMN {col} REAL"))
            except:
                pass
            ...
            for etf, col in [('XLU', 'benchmark_xlu_5d'), ('XLE', 'benchmark_xle_5d'), ('SPY', 'benchmark_spy_5d')]:
                try:
                    ...
                    if len(future) >= 5:
                        ...
                        session.execute(
                            text(f"UPDATE discovery_picks SET {col}=:p0 WHERE scan_date=:p1"),
                            {"p0": ret, "p1": sd}
                        )
                except Exception as e:
