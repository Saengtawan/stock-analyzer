...
def get_sector_map(conn: object, symbols: list[str]) -> dict[str, str]:
    ...
    rows = conn.execute(
        f"SELECT symbol, sector FROM sector_cache WHERE symbol IN ({placeholders})",
        symbols
    ).fetchall()
...
def insert_snapshots(conn: object, rows: list[dict]):
    ...
    conn.executemany("""
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, datetime('now'))
    """, [
        (r['date'], r['time_et'], r['symbol'], r['price'], r['volume'],
         r['vwap'], r['open'], r['high'], r['low'],
         r.get('signal_source'), r.get('action_taken'),
         r.get('scan_price'), r.get('pct_from_scan'),
         r.get('spy_price'), r.get('vix_at_time'),
         r.get('unrealized_pnl_pct'), r.get('pct_to_sl'),
...
def main():
    ...
    try:
        rows_pos = conn.execute(
            "SELECT symbol, entry_price, stop_loss FROM active_positions"
        ).fetchall()
        for p in rows_pos:
            active_pos[p['symbol']] = {'entry_price': p['entry_price'], 'stop_loss': p['stop_loss']}
    except Exception:
        pass
