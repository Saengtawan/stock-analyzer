...
def fill():
    with get_session() as session:
        ...
        for r in rows:
            by_symbol[r[1]].append({'id': r[0], 'symbol': r[1], 'sell_date': r[2], 'sell_price': r[3]})
            ...
            try:
                ...
                for row in by_symbol.get(sym, []):
                    ...
                    if updates:
                        ...
                        session.execute(text(f"UPDATE sell_outcomes SET {set_parts}, updated_at=datetime('now') WHERE id=:row_id"), params)
                        filled += 1
