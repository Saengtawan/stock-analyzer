...
For each sector ETF moving ≥ 1.5% from prior close:
  → Find top 10 individual stocks in that sector (by |pct_from_open| × volume)
...
def _get_sector_symbols(conn: sqlite3.Connection, sector_name: str,
    ...
    Filter by dollar_vol (proxy for avg volume × price).
    """
    rows = conn.execute("""
        SELECT sc.symbol
        ...
        LIMIT 100
    """, (sector_name, min_price * min_avg_vol)).fetchall()
...
def main():
    ...
    if not hot_sectors:
        ...
        for etf, data in etf_data.items():
            ...
            rows_to_insert.append((
                today, time_str, etf, sector_name,
                data['pct_chg'], data['today_price'],
    ...
    for etf, etf_info in hot_sectors.items():
        ...
        syms = _get_sector_symbols(conn, sector_name)
        if not syms:
        ...
        for rank, (sym, data) in enumerate(ranked[:TOP_N_PER_SECTOR], start=1):
            ...
            # pct_from_close = pct relative to yesterday's close (via ETF's prev_close proxy)
            # For individual stocks, compute from open as best proxy
            rows_to_insert.append((
                today, time_str,
                etf, sector_name, pct_chg, etf_info['today_price'],
                sym, data['price'], data['open_price'], data['pct_from_open'],
                None,           # pct_from_close — would need prior daily bar; skip for speed
                data['volume'], avg_vol, vol_ratio, rank,
            ))
