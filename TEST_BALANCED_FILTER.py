...
class ProductionScreener:
    ...
    def _analyze_stock(self, symbol: str, data: pd.DataFrame, market_regime: str) -> Optional[Dict]:
        ...
        # ========================================
        # BALANCED FILTER (Root Cause Fix)
        # Only use the MOST IMPORTANT filter
        # ========================================
        # 1. Must be above SMA20 (92% of losers were below)
        if current_price < sma20:
            return None

        # 2. No severe gap down (> -2% is too risky)
        if gap_pct < -2.0:
            return None

        # NOTE: ไม่ใช้ RSI filter เพราะกรอง winners เยอะเกินไป
