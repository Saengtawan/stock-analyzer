#!/usr/bin/env python3
"""
Growth Catalyst v3.1 - BULL MOMENTUM FILTERS (แก้ปัญหา Nov 3!)

🎯 เป้าหมาย: 10-15% กำไรต่อเดือน

🔍 Root Cause ที่เจอ:
- v3.0 เลือกหุ้นดี (RS +13.5%, Vol 44.6%) แต่ยังแพ้
- ปัญหา: เข้าตอน BULL ที่กำลัง "rolling over" (กำลังอ่อนแรง)
- ตัวอย่าง: Nov 3 เป็น BULL แต่มี warning signs:
  * SPY 5d: -0.46% (momentum ติดลบ!)
  * RSI: 68.4 (overbought!)
  * Strength: 50 (อ่อนแรง)

✨ v3.1 เพิ่มอะไร:
1. ✅ BULL Momentum Filters - เช็คคุณภาพของ BULL market
   - SPY 5-day > 0% (momentum ต้องเป็นบวก)
   - Regime strength >= 60 (BULL ต้องแข็งแรง ไม่ใช่แค่ 50)
   - RSI < 65 (ไม่ overbought)

2. ✅ เก็บ v3.0 ที่ดีทั้งหมดไว้:
   - Strict entry filters (RS>5%, Vol>35%, Momentum>8%)
   - Adaptive TP/SL
   - Smart peak detection

📊 คาดว่า:
- Nov 3 จะถูก BLOCK (มี warning signs)
- เหลือแค่ Oct 27 (BULL แข็งแรง) → Win rate 70%!
"""
...
class BullMomentumFilters:
    """เช็คคุณภาพของ BULL market - ไม่ใช่แค่ชื่อ BULL แต่ต้องมี momentum!"""
    ...
    FILTERS = {
        ...
        # v3.1 NEW! - BULL market quality filters
        'regime_strength_min': 60,      # BULL ต้องแข็งแรง (ไม่ใช่ 50)
        'spy_5d_min': 0.0,              # SPY momentum ต้องเป็นบวก
        'spy_rsi_max': 65.0,            # ไม่ overbought
    }
...
class BacktestV31BullMomentum:
    """Backtest v3.1 - เข้าแต่ BULL ที่มี momentum!"""
    ...
    def __init__(self, lookback_months=2):
        ...
        self.entry_filters = BullMomentumFilters.FILTERS
        ...
        print(f"   Strategy: v3.1 - เข้าแต่ BULL ที่มี MOMENTUM!")
    ...
    def check_bull_momentum_entry(self, symbol: str, entry_date: datetime) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            # v3.1 NEW! - Check BULL QUALITY
            spy_details = regime_info['details']

            # 1. Regime strength must be >= 60 (not weak 50)
            ...
            # 2. SPY 5-day momentum must be positive
            if spy_details['ret_5d'] < self.entry_filters['spy_5d_min']:
                return False, {'reason': f"SPY 5d negative ({spy_details['ret_5d']:.2f}%)"}

            # 3. RSI must not be overbought
            if spy_details['rsi'] > self.entry_filters['spy_rsi_max']:
                return False, {'reason': f"RSI overbought ({spy_details['rsi']:.1f})"}

            # 4. SPY trend still good
    ...
    def run_backtest(self):
        ...
        print("\n" + "="*100)
        print("🧪 BACKTEST v3.1 - BULL MOMENTUM FILTERS")
        print("="*100)
        ...
        print(f"\n   v3.1 NEW! - BULL Market Quality:")
        print(f"      - Regime Strength: >= {self.entry_filters['regime_strength_min']}")
        print(f"      - SPY 5d: > {self.entry_filters['spy_5d_min']:.0f}%")
        print(f"      - SPY RSI: < {self.entry_filters['spy_rsi_max']:.0f}")
    ...
    def analyze_results(self, trades: List[Dict]) -> Dict:
        ...
        print("\n" + "="*100)
        print("📊 BACKTEST RESULTS - v3.1 BULL MOMENTUM")
        print("="*100)
...
def main():
    print("🚀 Starting v3.1 Bull Momentum Backtest...")
    print("   Testing over 6 months to include sustained BULL periods")
