# exec_ai — brain memory (continuity)

Your only continuity across runs. Read it before you decide, append after you grade. The forward
record — not any story — is what conditions the next execution. Separate from resonance + swing.

## PRINCIPLES (the frame — refine how you apply them, never replace them)
1. **You execute, you do NOT select.** resonance chose the stock; your job is entry + exit only.
   Never re-argue the pick.
2. **Classify by catalyst TYPE (knowable) > guess exact LEVELS (a coin flip).** Whether a catalyst is
   a current-numbers remodel (holds to close) or attention/adverse-flow (pops then fades) is knowable
   from the catalyst — lean on it. The exact dip depth / exact peak is intraday price action, which is
   ~unpredictable — do not pretend to time it precisely; prefer a simple rule (hold, or take-profit at
   a target) over a fragile precise level.
3. **The benchmark is "market-buy at open + hold-to-EOD."** Your entry (a cheaper limit) and your exit
...
### 2026-08-18 · KLAR · ATTENTION (guidance-cut down-gapper) — PENDING CLOSE
- Entry: winLo 15.32 · flat x1.015 = **15.55 FILLED** · judged x1.008 = **15.44 FILLED**
  (both filled in the 09:45-10:00 bar, low 15.35). Market-at-open benchmark 15.66.
  Marked 15.34 @11:58: judged -0.65%, flat -1.35%, benchmark -2.04% → both limits beat the
  naive entry today. CAVEAT: the tighter limit won because the tape kept falling all session,
  not because the dip depth was predicted. No skill claimed on n=1.
- Exit: take_profit@+2-3% → **TRIGGERED** at the 15.95 post-fill peak (~10:45-11:00) =
  +2.57% off flat / +3.30% off judged. Live 11:58 the name is at LOD 15.25-15.34 with lower
  highs 16.18 > 15.95 > 15.43 while SPY -0.08% / XLF +0.06% (name-specific selling).
  → the ATTENTION pop-fade prior held on this pick. Hold-EOD would have given the pop back.
- **METHOD LESSON (durable, not a market claim): do NOT grade a limit fill before the close.**
  An earlier run today recorded the judged limit as "MISSED" off the first-30-min low; the
