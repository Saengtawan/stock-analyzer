#!/usr/bin/env python3
"""
PULLBACK CATALYST SYSTEM - PRODUCTION VERSION
เป้าหมาย: 8-12% ต่อเดือน (realistic target)

Balanced approach:
- Quality filters (80%+ WR)
- Good position sizing (25-30%)
- 5 sectors for enough trades
- 5 max positions
...
def run_production(
    ...
    """Production-ready Pullback Catalyst System"""

    if verbose:
        print("="*70)
        print("🚀 PULLBACK CATALYST SYSTEM - PRODUCTION VERSION")
        print("="*70)
        print("Realistic Target: 8-12% Monthly | 80%+ Win Rate")
        print("="*70)
    ...
    # Load data
    if verbose:
        print(f"Loading stocks from {len(sectors)} sectors...")
    ...
    if verbose:
        ...
        std_monthly = np.std(returns) if returns else 0

        print(f"\n  📊 Avg Monthly: {avg_monthly:+.2f}%")
        print(f"  📊 Median Monthly: {median_monthly:+.2f}%")
        print(f"  📊 Std Dev: {std_monthly:.2f}%")
        ...
        annual_return = (final_value / initial_capital) ** (12 / len(returns)) - 1
        ...
        print("="*70)
        print(f"  ✅ Win Rate: {win_rate:.0f}% (Target: 80%+)")
        print(f"  ✅ Profit Factor: {profit_factor:.2f} (Target: 2.0+)")
        print(f"  📊 Avg Monthly: {avg_monthly:.1f}%")
        print(f"  📊 Positive Months: {positive/len(returns)*100:.0f}%")

        if avg_monthly >= 8:
            print(f"\n  ✅ PRODUCTION READY - Realistic {avg_monthly:.1f}% monthly average")
        else:
            print(f"\n  ⚠️ Below realistic target of 8%")
    ...
    return {
        ...
        'positive_months_pct': positive/len(returns)*100 if returns else 0,
        'annual_return': annual_return if 'annual_return' in dir() else None,
    }
...
if __name__ == '__main__':
    ...
    if result:
        summary = f"""
PULLBACK CATALYST SYSTEM - PRODUCTION SUMMARY
=============================================
...
- Total Return: {result['total_pnl']/100000*100:.1f}%
- Avg Monthly: {result['avg_monthly']:.1f}%
- Positive Months: {result['positive_months_pct']:.0f}%

Strategy Rules:
1. CATALYST DETECTION: Volume spike > 1.9x with breakout/momentum
2. PULLBACK ENTRY: Wait for pullback to MA10 or ATR support
3. SCALED EXITS: T1 (5%), T2 (8.5%), T3 (13%) with trailing stops
4. STOP LOSS: -2.5% from entry
5. TIME STOP: 9-14 days if not performing

Sectors: Finance_Banks, Healthcare_Pharma, Semiconductors, Tech_Software, Consumer_Discretionary

Position Sizing:
- Strong Catalyst (score >= 65): 30% of capital
- Normal Catalyst (score >= 55): 25% of capital
- Weak Catalyst: 22.5% of capital
