"""
Consolidation Breakout (10:45-13:00) — tight range breakout.

Thesis: During the "dead zone" some stocks consolidate in tight ranges
(high-low < 1%) before breaking out as afternoon action begins.
Catch the breakout with volume confirmation.

⚠️ Not backtest-v2 validated. Paper trade first.

Setup:
- Stock intraday range <2% over the observation period
- Current price breaking above recent high with volume
- SPY green + AD >= 2 (hard gates)
- Avoid if already ran +5%+ today (no more room)
"""
import os
import sqlite3
import requests
...
class ConsolidationBreakStrategy(BaseStrategy):
    ...
    description = "10:45-13:00 tight-range consolidation breakout"
    expected_wr = 0.52
    expected_ev = 0.003
    time_start = "10:45"
    time_end = "13:00"
    ...
    MIN_PRICE = 3.0
    MAX_RANGE_PCT = 2.0      # range < 2% = tight
    MIN_BREAK_PCT = 0.3      # break above recent range
    MIN_DAILY_GAIN = 0.5
    MAX_DAILY_GAIN = 5.0
    MIN_BETA = 1.0
    MAX_BETA = 2.5
    ...
    def scan(self) -> ScanResult:
        ...
        for sym in syms:
            ...
            if range_pct > self.MAX_RANGE_PCT:
                continue
            # Must be gaining, not losing
            if not (self.MIN_DAILY_GAIN <= daily_gain <= self.MAX_DAILY_GAIN):
                continue
            # Breaking at high (within 0.3% of intraday high)
            if break_vs_high < -0.3:
                continue
            ...
            sl_pct = -max(self.SL_MIN_PCT, 0.5 * range_pct)
            tp_pct = max(self.TP_MIN_PCT, 1.5 * range_pct)
            sl_price = now * (1 + sl_pct / 100)
            tp_price = now * (1 + tp_pct / 100)

            sec = sectors.get(sym, '')
            reason = f"Tight{range_pct:.1f}% gain+{daily_gain:.1f}% atHi{break_vs_high:+.1f}% β{beta:.1f} {sec[:6]}"

            pick = Pick(
                symbol=sym, entry=now,
                sl_price=round(sl_price, 2), tp_price=round(tp_price, 2),
                trail_pct=self.TRAIL_PCT, reason=reason, score=5, atr_pct=range_pct,
                extra={
        ...
        if not candidates:
            return self.no_picks("No consolidation breakouts (need tight range + breaking high)")

        candidates.sort(key=lambda p: (p.extra['range_pct'], -p.extra['daily_gain']))
        sec_count = {}
        picks = []
        for c in candidates:
            sec = c.extra['sector']
        ...
        return ScanResult(
            ...
            reason=f"{len(candidates)} tight-range breakouts → top {len(picks)}",
            picks=picks,
