#!/usr/bin/env python3
"""
Check stocks passing v6.1 criteria today
"""
...
def check_v61(m):
    """Check v6.1 criteria and return status"""
    if m is None:
...
print("="*70)
print("STOCKS PASSING v6.1 TODAY")
print("="*70)
print("""
v6.1 Criteria:
  1. Above MA20
  ...
  4. Mom 3d: 1-8%
  5. RSI < 65
  6. Dist from 20d High >= -5% (not pullback)
""")

print("Checking stocks...\n")

passed = []
failed_close = []  # Almost passed
...
print(f"PASSED v6.1: {len(passed)} stocks")
print("="*70)

if passed:
    print(f"\n{'Symbol':<8} {'Price':>10} {'Mom20d':>8} {'Mom3d':>8} {'RSI':>6} {'52wPos':>8} {'Dist20d':>8}")
    print("-"*70)

    for s in passed:
        print(f"{s['sym']:<8} ${s['price']:>9.2f} {s['mom_20d']:>+7.1f}% {s['mom_3d']:>+7.1f}% {s['rsi']:>5.0f} {s['pos_52w']:>7.0f}% {s['dist_20d']:>+7.1f}%")
else:
    print("\nNo stocks passed all v6.1 criteria today.")

# Show almost passed
if failed_close:
    print(f"\n\nALMOST PASSED (failed 1 gate): {len(failed_close)} stocks")
    print("-"*70)
    print(f"{'Symbol':<8} {'Reason':<30} {'Mom20d':>8} {'Mom3d':>8} {'RSI':>6}")
    print("-"*70)

    for s in failed_close[:10]:
        print(f"{s['sym']:<8} {s['reason']:<30} {s['mom_20d']:>+7.1f}% {s['mom_3d']:>+7.1f}% {s['rsi']:>5.0f}")
