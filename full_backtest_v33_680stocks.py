#!/usr/bin/env python3
"""
FULL REALISTIC BACKTEST v3.3 - WITH 680+ STOCKS
================================================
EXACT SAME v3.3 LOGIC that achieved +8.23%/month
BUT with 680+ stocks universe (as production uses)
AND 6 months test period (more realistic)

v3.3 Logic (unchanged):
- Score >= 90 (high quality)
- 2 positions, 40% size
- Trail activation at +3%
- Trail 60% of gains
...
FULL_UNIVERSE = [
    ...
    'WDAY', 'MDB', 'CFLT', 'PATH', 'S', 'HUBS', 'VEEV', 'ANSS',
...
class HistoricalScreenerV33:
    """EXACT v3.3 Screener Logic - NO CHANGES"""

    # v3.3 Parameters - EXACTLY AS BEFORE
    MIN_ATR_PCT = 2.5
    MIN_SCORE = 90
    BASE_TP_PCT = 6.0
    BASE_SL_PCT = 3.5
    ...
    def load_historical_data(self, end_date: datetime, days: int = 60):
        ...
        for i in range(0, len(self.universe), batch_size):
            ...
            try:
                data = yf.download(batch, start=start_date, end=end_date + timedelta(days=1),
    ...
    def _analyze_stock(self, symbol: str, data: pd.DataFrame, market_regime: str) -> Optional[Dict]:
        """EXACT v3.3 Analysis - NO CHANGES"""
        if len(data) < 30:
            return None
        ...
        # v3.3 SCORING - EXACT SAME WEIGHTS
        score = 0
        reasons = []
...
class FullBacktestV33_680:
    """v3.3 Backtest with 680+ stocks - EXACT SAME PARAMETERS"""
    ...
    def __init__(self):
        ...
        self.screener = HistoricalScreenerV33(FULL_UNIVERSE)
    ...
    def run_backtest(self, months_back: int = 6):
        print(f"\n{'='*70}")
        print(f"FULL REALISTIC BACKTEST v3.3 - 680+ STOCKS")
        print(f"{'='*70}")
        print(f"\nSettings (EXACT SAME as v3.3):")
        ...
        print(f"  Trail Activation: +{self.TRAIL_ACTIVATION}%")
        print(f"  Trail Lock: {self.TRAIL_PERCENT}%")
        print(f"{'='*70}\n")
        ...
        while current <= end_date - timedelta(days=7):
            ...
            print(f"  Loading data for {len(FULL_UNIVERSE)} stocks...")
    ...
    def _show_results(self):
        ...
        print(f"\n{'='*70}")
        print(f"BACKTEST RESULTS - v3.3 with 680+ stocks")
        print(f"{'='*70}")
        ...
        for reason, count in sorted(exit_counts.items(), key=lambda x: -x[1]):
            ...
            print(f"  {reason:15}: {count:3} ({pct:5.1f}%) avg {avg:+.2f}% WR={win_count/count*100:.0f}%")
...
if __name__ == "__main__":
    backtest = FullBacktestV33_680()
    backtest.run_backtest(months_back=6)
