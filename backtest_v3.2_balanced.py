#!/usr/bin/env python3
"""
Growth Catalyst v3.2 - BALANCED BULL FILTERS (แก้ปัญหา v3.1 เข้มเกินไป!)

🎯 เป้าหมาย: 10-15% กำไรต่อเดือน

🔍 ปัญหา v3.1:
- Filters เข้มเกินไป! ได้แค่ 8 trades ใน 6 เดือน
- Block BULL market จริงๆ เพราะ:
  * RSI < 65 → Block BULL แข็งแรง (RSI 70-80 เป็นเรื่องปกติ!)
  * Strength >= 60 → Block BULL recoveries
  * SPY 5d > 0% → Block healthy pullbacks

✨ v3.2 ปรับอะไร:
1. 🎯 BALANCED BULL Filters - ไม่เข้มเกินไป!
   - RSI < 75 (แทน 65) → อนุญาต BULL แข็งแรง, block extreme overbought
   - Strength >= 50 (แทน 60) → อนุญาต BULL recoveries
   - SPY 5d > -1.5% (แทน 0%) → อนุญาต healthy pullbacks
   - NEW: SPY 20d > 0% → ต้องการ overall uptrend

2. ✅ เก็บ v3.0 ที่ดีทั้งหมดไว้:
   - Strict stock filters (RS>5%, Vol>35%, Momentum>8%)
   - Adaptive TP/SL
   - Smart peak detection

📊 คาดว่า:
- จะเข้า BULL markets จริงๆ (June-July, Aug-Sept)
- Win rate ควรเพิ่มขึ้นเพราะมี trades มากขึ้น
- ยังคง block Nov 3 (SPY 5d: -0.46% < -1.5% ไม่ใช่ แต่จะผ่าน...)
"""
...
class BalancedBullFilters:
    """v3.2 - BALANCED filters ที่ไม่เข้มเกินไป!"""
    ...
    FILTERS = {
        ...
        # v3.2 BALANCED - BULL market quality filters
        'regime_strength_min': 50,      # อนุญาต BULL recoveries (แก้จาก 60)
        'spy_5d_min': -1.5,             # อนุญาต healthy pullbacks (แก้จาก 0.0)
        'spy_20d_min': 0.0,             # NEW! Overall trend ต้องขึ้น
        'spy_rsi_max': 75.0,            # อนุญาต strong BULL (แก้จาก 65)
    }
...
class BacktestV32Balanced:
    """Backtest v3.2 - BALANCED BULL filters!"""
    ...
    def __init__(self, lookback_months=2):
        ...
        self.entry_filters = BalancedBullFilters.FILTERS
        ...
        print(f"   Strategy: v3.2 - BALANCED BULL Filters!")
    ...
    def check_bull_momentum_entry(self, symbol: str, entry_date: datetime) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            # v3.2 BALANCED - Check BULL QUALITY (ไม่เข้มเกินไป!)
            ...
            # 4. RSI < 75 (อนุญาต strong BULL, block extreme overbought)
            if spy_details['rsi'] > self.entry_filters['spy_rsi_max']:
                return False, {'reason': f"RSI extreme overbought ({spy_details['rsi']:.1f})"}
    ...
    def run_backtest(self):
        ...
        print("\n" + "="*100)
        print("🧪 BACKTEST v3.2 - BALANCED BULL FILTERS")
        print("="*100)
        ...
        print(f"\n   v3.2 BALANCED - BULL Market Quality:")
        print(f"      - Regime Strength: >= {self.entry_filters['regime_strength_min']} (relaxed from 60)")
        print(f"      - SPY 5d: > {self.entry_filters['spy_5d_min']:.1f}% (relaxed from 0%)")
        print(f"      - SPY 20d: > {self.entry_filters['spy_20d_min']:.0f}% (NEW!)")
        print(f"      - SPY RSI: < {self.entry_filters['spy_rsi_max']:.0f} (relaxed from 65)")
    ...
    def analyze_results(self, trades: List[Dict]) -> Dict:
        ...
        print("\n" + "="*100)
        print("📊 BACKTEST RESULTS - v3.2 BALANCED BULL FILTERS")
        print("="*100)
...
def main():
    print("🚀 Starting v3.2 BALANCED Bull Filters Backtest...")
    print("   Testing over 6 months with RELAXED filters")
    print("   Should allow June-July and Aug-Sept BULL markets")
    print("   This will take 5-10 minutes...\n")

    backtest = BacktestV32Balanced(lookback_months=6)
